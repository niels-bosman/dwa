...
game.goToLocation = (locationName, done) => {
  if (!game.getLocationInformation().exits.includes(locationName)) {
    return done('Cant go to this place', locationName);
  }

  return request(`http://localhost:3000/${locationName}`, null, (error, response, body) => {
    ...
    return done(null, location.description);
  });
};
